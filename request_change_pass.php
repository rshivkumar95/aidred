...
try
{
       ...
       if($_POST['newpassword']===$_POST['repassword'])
       {
            ...
            $res = $coll->findOne(array('blood_request.username' => $_SESSION['username'], 'blood_request.password' =>$_POST['oldpassword']));
            if($res)
             {
                $res1 = $coll->update(array('blood_request.username' =>$_SESSION['username']),array('$set'=>array('blood_request.password' => $_POST['newpassword'])));
                if($res1)
                  echo "<script type='text/javascript'>alert('Password Changed !'); window.location.href='dashboard.php';</script>";  
